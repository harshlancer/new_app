...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, ImageBackground, TextInput, Alert, ActivityIndicator } from 'react-native';
...
export default function GuestDining({ navigation }) {
    const { cart, addToCart, decreaseQty, itemCount } = useCart();
    const [activeCat, setActiveCat] = useState('All');
    const [menuItems, setMenuItems] = useState([]);
    const [loading, setLoading] = useState(true);
    const categories = ['All', 'Breakfast', 'Lunch', 'Dinner', 'Evergreen', 'Drinks', 'Dessert'];
    ...
    const filtered = activeCat === 'All' ? menuItems : menuItems.filter(it => it.category === activeCat);

    const handleAdd = (item) => {
        addToCart(item);
        showToast(`${item.name} added to your tray!`, 'success');
    };
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.list}>
                {loading ? (
                    <ActivityIndicator size="large" color="#ab9373" style={{ marginTop: 50 }} />
                ) : (
                    filtered.map(item => {
                        ...
                        <View style={styles.cardContent}>
                            ...
                            {cartItem ? (
                                <View style={styles.inlineQtyBox}>
                                    <TouchableOpacity style={styles.inlineQtyBtn} onPress={() => decreaseQty(item.id)}>
                                        <Icon name="minus" size={18} color="#ab9373" />
                                    </TouchableOpacity>
                                    <Text style={styles.inlineQtyText}>{cartItem.qty}</Text>
                                    <TouchableOpacity style={styles.inlineQtyBtn} onPress={() => addToCart(item)}>
                                        <Icon name="plus" size={18} color="#ab9373" />
                                    </TouchableOpacity>
                                </View>
                            ) : (
...
const styles = StyleSheet.create({
    ...
    inlineQtyBox: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fdfbf7', padding: 10, borderRadius: 12, marginTop: 20, borderWidth: 1, borderColor: '#e8e4d9', gap: 20 },
    inlineQtyBtn: { padding: 4 },
    inlineQtyText: { fontSize: 18, color: '#4a3b2c', fontWeight: '800' },
